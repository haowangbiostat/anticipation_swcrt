...
Qs <- c(3, 5, 7)
df_HH <- do.call(rbind, lapply(Qs, run_simulation, coeff_HH))
df_HH_ANT <- do.call(rbind, lapply(Qs, run_simulation, df_HH_ANT))

custom_colors <- brewer.pal(n = 7, name = "Set1")
...
ggsave(filename = "../figures/figure_coeff_HH_vs_HH-ANT.pdf", plot = p3, width = 12, height = 5)