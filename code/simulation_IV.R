...
gamma = 0.04 # anticipation effect
...
while (sim_count < n_sim) {
  ...
  result_sim <- try({
    ...
    if (sim %% batch_size == 0) {
      file_name <- sprintf("../result/simulation_IV/simulation_results_%04d_to_%04d.csv", sim - batch_size + 1, sim)
