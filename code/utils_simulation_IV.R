...
scale2 <- function(x, delta, sd_expt) {
  return(sd_expt*(x-mean(x))/sd(x) + delta)
}
...
gendata <- function(
  ...
  sd_expt = 1 # standard deviation of exposure time het
  e <- 1:(nperiod-1)
  true <- -0.5*sin(2*pi*(e-1)/(nperiod-2)) + log(1.2)
  true = scale2(true, delta, sd_expt)
  theta = c(0, true)
  ...
  dat <- dat %>%
    mutate(response.var = response.var + theta[expt.var + 1])
...
s2_get_nve_a <- function(m1, m2, m3, m4, m5, m6, nperiod){
  ...
  nai_HH_ANT <- sqrt(vcov(m2)[3, 3])
  nai_ETI <- NA
  nai_ETI_ANT <- sqrt(vcov(m4)[nperiod+1, nperiod+1])
  nai_TEH <- NA
