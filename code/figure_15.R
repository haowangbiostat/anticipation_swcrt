...
for (panel_lab in levels(df_all$panel_id)) {
  ...
  panel_title <- bquote(
    .(panel_lab) ~ " " ~ Power^{"ETI-ANT"} / Power^{"ETI"} ~
      "(" ~ J == .(J_cur) ~ ", " ~ Delta^{"ETI-ANT"} == .(Delta_cur) ~ ")"
  )
  ...
  p <- ggplot(df_sub, aes(x = rho, y = gamma_over_Delta, z = ratio)) +
    ...
    labs(
      x = expression(rho),
      y = expression(frac(gamma^{"ETI-ANT"}, Delta^{"ETI-ANT"})),
      title = panel_title
    ) +
    theme_bw() +
    theme(
...
pdf("../figures/figure_power_ratio_fixed_Delta.pdf", width = 14, height = 14, paper = "special")
grid.arrange(
  grobs = plots_list,
  nrow  = 3,
  ncol  = 2
