...
naive_power_t <- matrix(NA, nrow = n_sim, ncol = 2)
...
colnames(naive_power_t) <- c("HH", "HH-ANT")
...
while (sim_count < n_sim) {
  ...
  result_sim <- try({
    ...
    if (sim %% batch_size == 0) {
      file_name <- sprintf("../result/simulation_I (power)/simulation_results_%04d_to_%04d.csv", sim - batch_size + 1, sim)
